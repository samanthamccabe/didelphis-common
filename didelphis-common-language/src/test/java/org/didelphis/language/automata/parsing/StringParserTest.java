...
class StringParserTest {
	...
	private final LanguageParser<String> parser = new StringParser();
	...
	void testConcatenate() {
		assertEquals("ab", parser.concatenate("a", "b"));
	}
	...
	@Test
	void testGetDot() {
		Arc<String> dot = parser.getDot();
		assertEquals(1, dot.match("a", 0));
		assertEquals(parser.getArc("."), dot);
	}
	...
	class TestParsing {
		...
		void testSpecials01() {
			...
			specials.add("CH", "th");
			Expression ex1 = new StringParser(specials).parseExpression(
					"ataCHam");
